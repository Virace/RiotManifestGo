...
//
// installed.json 缺失、内容不是合法 JSON、或 schema 字段不等于当前识别的版本，
// 均视为"无有效状态"，返回 (nil, nil)（而非错误），以便调用方以此判断
// "本地没有可用于增量更新的旧版本锚点"。只有真正的 I/O 错误（如权限问题）才会
// 返回非 nil 的 error。
...
// 指向的）manifest 文件，其余全部删除，避免 manifests/ 目录无限增长。
func (a *Archive) Save(manifestID uint64, raw []byte, source string) error {
